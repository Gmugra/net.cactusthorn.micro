...
public interface HikariConf extends Config {
    ...
    @Key("hikari.driver") @DefaultValue("org.postgresql.Driver") //
    String driver();

    @Key("hikari.url") //
    String url();

    @Key("hikari.username") @EncryptedValue(PasswordDecryptor.class) //
    String username();

    @Key("hikari.password") @EncryptedValue(PasswordDecryptor.class) //
    String password();

    @Key("hikari.schema") //
    String schema();

    @Key("hikari.minimum-idle") @DefaultValue("1") //
    int minimumIdle();

    @Key("hikari.maximum-pool-size") @DefaultValue("5") //
    int maximumPoolSize();

    @Key("hikari.poolname") @DefaultValue("MainPool") //
    String poolname();
}
